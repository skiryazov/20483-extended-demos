...
namespace FourthCoffee.Employees
{
    class Program
    {
        ...
        private static void UsingAnonymousTypes()
        {
            ...
            var emps1 = from e in DBContext.Employees where e.DateOfBirth > DateTime.Now.AddYears(-18) select e;

            var emps2 = from e in emps1 where e.JobTitle.Value == DEVELOPER select e;

            var emps = from e in emps2 select new { e.FirstName, e.LastName };
